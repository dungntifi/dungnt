            ...
            if (i == keyParts.length-1)
            {
                for (x = 0; x < settings[i].options.length; x++)
                {
                    ...
                    if ('undefined' != typeof(this.options[keyCheck]))
                    {
                        ...
                        {
			    var optText = this.options[keyCheck]['custom_status'];
			    optText = optText.replace(/(<([^>]+)>)/ig,"");
                            if (!strpos(settings[i].options[x].text, optText))
                            {				
                                settings[i].options[x].text = settings[i].options[x].text + ' (' + optText + ')';
                            }
                        }
                    }
                }
