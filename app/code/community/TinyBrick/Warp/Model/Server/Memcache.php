...
class TinyBrick_Warp_Model_Server_Memcache
{
	protected $_server;
	...
	{            
		if(!$this->_server){
			if(($memcachedConfig = Mage::getStoreConfig('dfv/memcached')) && extension_loaded('memcache')){
				$this->_enabled = true;
				$this->_useCompression = true;
				$this->_server = new Memcache();
				$this->_server->addServer(
						 (string)$memcachedConfig['host']
						,(string)$memcachedConfig['port']
						,(string)$memcachedConfig['persistent']);
