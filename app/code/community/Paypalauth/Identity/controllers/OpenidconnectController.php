<?php
class Paypalauth_Identity_OpenidconnectController extends Mage_Core_Controller_Front_Action {
	...
	public function returnAction() {
		...
		$paypalCustomerData = Mage::getModel ( 'paypalauth_identity/paypal_customer' )->getPaypalCustomerDataByField ( 'payer_id', $profile->getUserId() );
		...
		if (! $paypalCustomerData) {
			...
			if ($this->_getSession ()->isLoggedIn ()) {
				...
				$url = Mage::getUrl ( 'login/openidconnect/askLink' );
	...
	public function askLinkPostAction() {
		...
		$askLinkUrl = 'login/openidconnect/askLink';
		...
		if (array_key_exists ( 'send', $params )) {
			...
			$isPaypalCustomerExists = Mage::getModel ( 'paypalauth_identity/paypal_customer' )->isPaypalCustomerExists ( 'customer_id', $customerId );
			...
			if ($url == 'login/openidconnect/askLink') {
				$this->_redirect ( $url );
				return;
			}
